...
class NounParserFactory:
  ...
  def parser( cls, attr, **kwargs ):
    ...
    type = re.split(r'\|(?=\|*)', attr)[0][8:]

    if not type or not cls.parsers.get( type ):
      raise KeyError
    else:
      return cls.parsers.get( type )
...
if __name__ == '__main__':
  try:
    ...
    file = open('noun.txt', 'w', encoding='utf-8')

    for o in sorted(output, key=lambda x: x[2]):
      file.write(o[2] + ': ' + str(o[3]['word']) + '\n')
    file.close()